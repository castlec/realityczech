...
class PageParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "img":
            ...
            if self._anchor is not None and alt:
                self._anchor.images.append(alt)
        for attribute in (
            "src",
            "data-src",
            "data-lazy-src",
            ...
            if values.get(attribute):
                self.media_urls.append((values[attribute], f"{tag}:{attribute}"))
    ...
    def handle_data(self, data: str) -> None:
        ...
        if self._anchor is not None:
            self._anchor.text = f"{self._anchor.text} {cleaned}".strip()
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=60) as response:
                ...
                if not 200 <= status < 300:
                    raise DiscoveryError(f"HTTP {status}")
...
def docx_export_url(doc_id: str) -> str:
...
def classify_url(url: str) -> str:
    ...
    if suffix in IMAGE_EXTENSIONS:
        return "image"
    if parsed.netloc in {"youtube.com", "www.youtube.com", "youtu.be"} or YOUTUBE_ID_RE.search(url):
        return "youtube"
    if parsed.netloc in {"drive.google.com", "drive.usercontent.google.com"}:
        return "google-drive"
    if parsed.netloc == "docs.google.com":
        return "google-document"
    if parsed.netloc == "utexas.instructure.com":
        return "canvas"
    if "quizlet.com" in parsed.netloc:
        return "quizlet"
    return "link"
...
def attribution_context(text_parts: list[str], anchors: list[Anchor]) -> dict[str, Any]:
    ...
    for anchor in anchors:
        ...
        if start >= 0 or any(
            ...
            for token in ("credit", "source", "license", "creative commons", "forvo")
        ):
...
def parse_html_page(url: str, data: bytes) -> dict[str, Any]:
    ...
    anchors = [
        {
            "url": normalize_url(absolute(url, anchor.href)),
            "label": " ".join([anchor.text, *anchor.images]).strip(),
        }
        for anchor in parser.anchors
        if anchor.href and not anchor.href.startswith(("mailto:", "javascript:", "#"))
    ]
    media = [
        {"url": normalize_url(absolute(url, value)), "discoveredBy": method}
        for value, method in parser.media_urls
...
def relevant_unit_link(anchor: dict[str, str]) -> bool:
    parsed = urllib.parse.urlsplit(anchor["url"])
    if parsed.netloc not in COURSE_HOSTS:
        return False
    if parsed.netloc in FIRST_PARTY_HOSTS:
        return bool(parsed.path) and parsed.path not in NAVIGATION_PATHS
    return True
...
def source_attribution(page: dict[str, Any]) -> dict[str, Any]:
    ...
    if not text and urllib.parse.urlsplit(page["url"]).netloc in FIRST_PARTY_HOSTS:
        text = (
            "Reality Czech by Christian Hilchey and COERLL; Creative Commons. "
            "Preserve item-level credits from the source page or document."
        )
